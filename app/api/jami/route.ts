...
${NO_MARKDOWN_RULE}`,
    ...
    if (truncated) console.warn(`[jami] stop_reason=max_tokens chapterId=${chapterId} usage=${JSON.stringify(message.usage)}`);

    const summary = `명궁(${chart.soulPalaceBranch}) · 신궁(${chart.bodyPalaceBranch}) · ${chart.fiveElementsClass} · ${chart.zodiac}띠`;
    return NextResponse.json({ result: stripMarkdown(raw), chart: summary, chapterId, truncated });
  } catch (err) {
